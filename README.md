...
- **Formulaire de capture** : champ email + menu déroulant de profil
  (`Étudiant` / `Professionnel`, placeholder `> choisissez votre profil`).
- Le bouton **« Recevoir l'ebook »** ne s'active **que** si l'email est valide
  **et** un profil est sélectionné.
- Section **« Ce que tu vas apprendre »** (reformulation claire du sommaire).
- À la soumission, la route `POST /api/subscribe` relaie vers un **workflow n8n**
  ...
  2. enregistre **email + profil + date** dans **Google Sheets**.
...
- **Webhook** (`POST /webhook/ebook-prompt-engineering`) reçoit `{ email, profile, profileLabel }`.
- **Google Sheets** : ajoute une ligne `date | email | profil`.
